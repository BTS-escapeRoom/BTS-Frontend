...
type ThemeInfoButtonProps = {
  theme: ThemeDetail
  onClick?: () => void
  className?: string
}

export default function ThemeInfoButton({ theme, onClick, className = '' }: ThemeInfoButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`mt-1 w-full rounded-[2px] bg-[#FAFAFA] p-2 text-left transition-opacity hover:opacity-80 ${className}`}
    >
      <div className="flex gap-2">
        ...
        <div className="flex flex-1 flex-col gap-1">
          ...
          <div className="flex flex-wrap items-center gap-[6px]">
            <SChip text={theme.genreType} />
            <DifficultyChip difficulty={theme.difficulty} />
            <SChip text={formatTimeLabel(theme.time)} icon={<IconClock />} />
          </div>

          {/* 세번째 줄: city와 store 명 */}
          <div className="text-12 text-gray06">{theme.store?.name}</div>
        </div>
      </div>
    </button>
  )
